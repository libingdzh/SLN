...
def NiiDataRead_2D(path, as_type=np.float32):
    nii = sitk.ReadImage(path)
    spacing = nii.GetSpacing()  # [x,y]
    volumn = sitk.GetArrayFromImage(nii)  # [y,x]
    if len(volumn.shape) == 3:
        volumn = volumn[0]
    origin = nii.GetOrigin()
    direction = nii.GetDirection()
    ...
    spacing_ = np.array([spacing_y, spacing_x])
    return volumn.astype(as_type), spacing_.astype(np.float32), origin, direction
